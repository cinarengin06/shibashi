...
export default function Practice(){
 ...
 const suggested=practices.find(x=>x.id===shen.practiceId)??practices[0];
 ...
 return <Screen>
  ...
  <View style={p.featureGrid}>
   <Pressable onPress={()=>router.push('/living-learning')} style={[p.feature,{borderColor:`${shen.color}55`}]}><View style={[p.featureIcon,{backgroundColor:`${shen.color}18`}]}><Ionicons name="sparkles-outline" color={shen.color} size={25}/></View><Text style={[p.featureLabel,{color:shen.color}]}>YAŞAYARAK ÖĞREN</Text><Text style={p.featureTitle}>Hikâyeyi yaşa</Text><Text style={p.featureMeta}>{completedStories.length}/18 sahne tamamlandı</Text><ProgressBar value={completedStories.length/18*100} color={shen.color}/></Pressable>
   <Pressable onPress={()=>router.push('/posture')} style={[p.feature,{borderColor:`${shen.color}55`}]}><View style={[p.featureIcon,{backgroundColor:`${shen.color}18`}]}><Ionicons name="scan-outline" color={shen.color} size={25}/></View><Text style={[p.featureLabel,{color:shen.color}]}>POSTÜR AYNASI</Text><Text style={p.featureTitle}>Beden çizgini gör</Text><Text style={p.featureMeta}>{postureReports.length?`Son skor ${postureReports[0].score}`:'İlk taramanı başlat'}</Text><ProgressBar value={postureReports[0]?.score??5} color={shen.color}/></Pressable>
  </View>
  ...
  {visiblePractices.length?visiblePractices.map(x=>{const done=sessions.some(s=>s.practiceId===x.id);return <Pressable key={x.id} disabled={x.locked} onPress={()=>router.push({pathname:'/practice-session',params:{practiceId:x.id}})}><Card style={[p.routine,x.locked&&{opacity:.45}]}><ImageBackground source={movementImages[movements.find(m=>m.id===x.movementIds[0])?.order??1]} style={p.cover} imageStyle={p.coverImage}><View style={p.coverShade}/><Ionicons name={x.locked?'lock-closed':done?'checkmark':'play'} color={colors.cream} size={22}/></ImageBackground><View style={{flex:1,gap:4}}><Text style={p.title}>{x.title}</Text><Text style={p.meta}>{x.duration} dk · {x.focus} · {x.movementIds.length} hareket</Text>{done&&<Text style={[p.doneText,{color:shen.color}]}>TAMAMLANDI</Text>}</View><Ionicons name="chevron-forward" color={colors.muted} size={20}/></Card></Pressable>}) : <Card><Text style={p.meta}>Bu odakta henüz bir rutin yok. 18 temel hareketten seçim yapabilirsin.</Text></Card>}
  ...
  {movements.map(x=><Pressable key={x.id} onPress={()=>router.push({pathname:'/movement/[id]',params:{id:x.id}})}><View style={p.movement}><ImageBackground source={movementImages[x.order]} style={p.movementImage} imageStyle={p.movementImageRadius}><View style={p.coverShade}/><Text style={p.order}>{String(x.order).padStart(2,'0')}</Text></ImageBackground><View style={{flex:1}}><Text style={p.title}>{x.name}</Text><Text style={p.meta}>{x.focus} · {x.duration} dk · Aşama {x.gate}</Text></View><Ionicons name="play-circle-outline" color={shen.color} size={25}/></View></Pressable>)}
 </Screen>;
}
const p=StyleSheet.create({
 ...
 shenLabel:{fontSize:9,fontWeight:'900',letterSpacing:1.2},
 suggestionTitle:{color:colors.cream,fontSize:27,fontWeight:'800',letterSpacing:-.5},
 suggestionMeta:{color:colors.muted,fontSize:12,marginTop:5},
 tradition:{color:'rgba(245,240,232,.62)',fontSize:9,lineHeight:14,marginTop:6},
 ...
 playText:{color:colors.ink,fontSize:13,fontWeight:'900'},
 ...
 featureLabel:{fontSize:8,fontWeight:'900',letterSpacing:1},
 featureTitle:{color:colors.cream,fontSize:15,fontWeight:'800',lineHeight:20},
 featureMeta:{color:colors.muted,fontSize:10,lineHeight:14,flex:1},
 ...
 chipText:{color:colors.cream,fontSize:13,fontWeight:'700'},
 section:{color:colors.gold,fontSize:11,fontWeight:'900',letterSpacing:1.5,marginTop:8},
 sectionRow:{flexDirection:'row',justifyContent:'space-between',alignItems:'center'},
 completed:{fontSize:10,fontWeight:'800'},
 ...
 title:{color:colors.cream,fontWeight:'800',fontSize:15},
 meta:{color:colors.muted,fontSize:12,lineHeight:17},
 doneText:{fontSize:8,fontWeight:'900',letterSpacing:1},
 ...
 order:{color:colors.cream,fontSize:12,fontWeight:'900'},
});

...
import {LivingGhostOverlay} from '../../components/living-learning/LivingPoseOverlay';
...
export default function LivingStoryScreen(){
 ...
 const[voiceEnabled,setVoiceEnabled]=useState(true),[preview,setPreview]=useState({width:1,height:1}),[feedback,setFeedback]=useState('Tam bedenin göründüğünde ölçüm başlayacak.'),[liveScore,setLiveScore]=useState<number|null>(null),[result,setResult]=useState<LivingPracticeResult|null>(null),[saved,setSaved]=useState(false);
 const cameraRef=useRef<CameraView|null>(null),bridgeRef=useRef<MediaPipePoseBridgeRef|null>(null),samplesRef=useRef<LivingComparisonResult[]>([]),stepRef=useRef(livingStepAt(0,practice)),elapsedRef=useRef(0),lastStepRef=useRef(stepRef.current.id),completionRef=useRef(false),lastVoiceRef=useRef({text:'',at:0});
 ...
 useEffect(()=>{
  if(mode!=='camera'||!running||!voiceEnabled)return;
  const text=`${activeStep.title}. ${activeStep.instruction}`;
  lastVoiceRef.current={text,at:Date.now()};void Speech.stop();Speech.speak(text,{language:'tr-TR',rate:.84,pitch:1});
 },[activeStep.id,activeStep.instruction,activeStep.title,mode,running,voiceEnabled]);
 useEffect(()=>{
  if(mode!=='camera'||!running||!voiceEnabled||liveScore!==null)return;
  const text='Telefonu sabitle ve iki üç metre geri çekil.';const now=Date.now();
  if(lastVoiceRef.current.text===text&&now-lastVoiceRef.current.at<9000)return;
  lastVoiceRef.current={text,at:now};void Speech.stop();Speech.speak(text,{language:'tr-TR',rate:.84,pitch:1});
 },[feedback,liveScore,mode,running,voiceEnabled]);
 ...
 useEffect(()=>{
  ...
  const analyze=async()=>{
   ...
   try{
    ...
    if(cancelled)return;
    const named=toNamedPose(analysis.landmarks);
 ...
 const resetTimeline=()=>{completionRef.current=false;samplesRef.current=[];setLiveScore(null);setFeedback('Tam bedenin göründüğünde ölçüm başlayacak.');setElapsedMs(0);lastStepRef.current='prepare'};
 ...
 const toggleVoice=()=>setVoiceEnabled(current=>{const next=!current;if(!next)void Speech.stop();return next});
 ...
 if(mode==='camera')return <View style={s.cameraRoot} onLayout={onPreviewLayout}>
  ...
  <View pointerEvents="none" style={live.teacherStage}><Text style={live.figureLabel}>ÖĞRETMEN</Text><LivingGhostOverlay step={activeStep} width={Math.max(176,preview.width*.48)} height={Math.max(360,preview.height*.56)}/></View>
  <View pointerEvents="none" style={[live.userTarget,liveScore!==null&&live.userTargetReady]}><Text style={[live.figureLabel,live.userLabel]}>SEN</Text>{liveScore!==null?<View style={live.readyBadge}><Ionicons name="checkmark-circle" color="#A9D977" size={17}/><Text style={live.readyBadgeText}>Tam beden hazır</Text></View>:null}</View>
  <SafeAreaView style={s.cameraSafe} edges={['top','bottom']} pointerEvents="box-none">
   <View style={s.cameraTop}><Pressable onPress={()=>{setRunning(false);setMode('watch');resetTimeline();void Speech.stop()}} style={s.round}><Ionicons name="close" color={colors.cream} size={21}/></Pressable><View style={s.cameraStep}><Text style={s.cameraStepCount}>{practice.steps.findIndex(item=>item.id===activeStep.id)+1} / 5</Text><Text style={s.cameraStepTitle}>{activeStep.title}</Text></View><Pressable onPress={toggleVoice} style={s.round}><Ionicons name={voiceEnabled?'volume-high':'volume-mute'} color={voiceEnabled?colors.gold:colors.muted} size={20}/></Pressable></View>
   <View style={s.cameraCenter}><Text style={s.breath}>Öğretmeni taklit et</Text></View>
   <View style={s.cameraBottom}><View style={[live.feedback,liveScore!==null&&live.feedbackReady]}><Ionicons name={liveScore===null?'resize-outline':'mic-outline'} color={liveScore===null?colors.gold:'#A9D977'} size={22}/><View style={live.feedbackCopy}><Text style={s.feedbackState}>{liveScore===null?'KADRAJI HAZIRLA':'HAREKETİ UYGULA'}</Text><Text style={s.feedbackText}>{liveScore===null?'Telefonu sabitle ve 2–3 metre geri çekil.':activeStep.instruction}</Text></View></View><View style={s.controls}><View style={s.progress}><View style={[s.progressFill,{width:`${elapsedMs/practice.durationMs*100}%`}]}/></View><Pressable onPress={()=>setRunning(value=>!value)} style={s.pause}><Ionicons name={running?'pause':'play'} size={20} color="#11150F"/></Pressable><Text style={s.time}>{Math.ceil((practice.durationMs-elapsedMs)/1000)} sn</Text></View></View>
 ...
 return <View style={s.watchRoot}>
  ...
  <View style={[s.demoGhost,live.demoGhost]}><LivingGhostOverlay step={activeStep} width={230} height={420}/></View>
...
const live=StyleSheet.create({teacherStage:{position:'absolute',zIndex:4,left:-2,top:'18%',width:'50%',height:'56%',alignItems:'center',justifyContent:'center'},figureLabel:{position:'absolute',top:0,zIndex:2,fontFamily:fonts.sansBold,color:'#F3CF8B',fontSize:13,letterSpacing:1.3,backgroundColor:'rgba(10,15,12,.62)',paddingHorizontal:10,paddingVertical:6,borderRadius:14,overflow:'hidden'},userLabel:{color:'#A9D977'},userTarget:{position:'absolute',zIndex:3,right:10,top:'22%',width:'45%',height:'53%',borderRadius:74,borderWidth:1.5,borderStyle:'dashed',borderColor:'rgba(242,238,231,.32)'},userTargetReady:{borderColor:'rgba(169,217,119,.78)',backgroundColor:'rgba(169,217,119,.035)'},readyBadge:{position:'absolute',bottom:-34,alignSelf:'center',flexDirection:'row',alignItems:'center',gap:6,backgroundColor:'rgba(10,15,12,.78)',borderRadius:18,paddingHorizontal:11,paddingVertical:7},readyBadgeText:{fontFamily:fonts.sansBold,color:'#A9D977',fontSize:14},feedback:{alignSelf:'stretch',minHeight:72,padding:13,borderRadius:18,borderWidth:1,borderColor:'rgba(198,165,106,.34)',backgroundColor:'rgba(10,15,12,.84)',flexDirection:'row',alignItems:'center',gap:11},feedbackReady:{borderColor:'rgba(169,217,119,.34)'},feedbackCopy:{flex:1},demoGhost:{top:'14%',left:'2%',opacity:.92}});

...
function ReportDetail({report,shen,onBack}:{report:PostureReport;shen:ReturnType<typeof getShen>;onBack:()=>void}){
 ...
 const hasPose=Boolean(capture.imageData&&capture.landmarks&&capture.landmarks.length>=33);
 ...
 return <Screen>
  ...
  <Card style={[p.detailVisualCard,{borderColor:`${shen.color}55`}]}><View style={p.detailVisual}>{hasPose?<CapturePoseOverlay capture={capture}/>:<View style={p.missingCapture}><Ionicons name="image-outline" color={colors.muted} size={28}/><Text style={p.missingCaptureText}>Bu kaydın kamera görüntüsü bu cihazda saklanmamış.</Text></View>}</View><View style={p.detailVisualCopy}><Text style={[p.detailViewEyebrow,{color:shen.color}]}>{viewLabels[activeView]} GÖRÜNÜM</Text><Text style={p.detailVisualTitle}>{capture.feedback}</Text><Text style={p.small}>33 nokta · {capture.sampleCount??1} geçerli kare · güven %{Math.round((capture.confidence??0)*100)}</Text>{hasPose&&<View style={p.pointLegend}><View style={p.legendItem}><View style={[p.legendDot,{backgroundColor:colors.jade}]}/><Text style={p.small}>İyi</Text></View><View style={p.legendItem}><View style={[p.legendDot,{backgroundColor:colors.gold}]}/><Text style={p.small}>Dikkat</Text></View><View style={p.legendItem}><View style={[p.legendDot,{backgroundColor:colors.danger}]}/><Text style={p.small}>Düzelt</Text></View></View>}</View></Card>
...
function CapturePoseOverlay({capture}:{capture:PostureCapture}){
 ...
 const toneFor=(index:number)=>index>=11&&index<=16?shoulderTone:index===23||index===24?hipTone:axisTone;
 return <View style={p.captureOverlay} pointerEvents="none"><Image source={{uri:capture.imageData}} resizeMode="stretch" style={p.captureImage}/><Svg width="100%" height="100%" viewBox="0 0 1 1" preserveAspectRatio="none" style={StyleSheet.absoluteFill}><Line x1=".5" x2=".5" y1=".08" y2=".94" stroke={axisTone} strokeWidth=".004" strokeDasharray=".012 .01" opacity=".8"/>{resultPoseConnections.map(([startIndex,endIndex])=>{const start=landmarks[startIndex],end=landmarks[endIndex];if(!start||!end||((start.visibility??0)<.3)||((end.visibility??0)<.3))return null;return <Line key={`${startIndex}-${endIndex}`} x1={start.x} y1={start.y} x2={end.x} y2={end.y} stroke={toneFor(startIndex)} strokeWidth=".008" strokeLinecap="round"/>})}{resultPosePoints.map(index=>{const point=landmarks[index];if(!point||((point.visibility??0)<.3))return null;return <Circle key={index} cx={point.x} cy={point.y} r=".018" fill={toneFor(index)} stroke="#F4F0E8" strokeWidth=".004"/>})}</Svg></View>;
}
...
const p=StyleSheet.create({
 captureOverlay:{...StyleSheet.absoluteFillObject},captureImage:{width:'100%',height:'100%'},missingCapture:{flex:1,width:'100%',alignItems:'center',justifyContent:'center',padding:30,gap:10},missingCaptureText:{color:colors.muted,textAlign:'center',fontSize:17,lineHeight:28},pointLegend:{flexDirection:'row',gap:12,marginTop:4},legendItem:{color:colors.muted,fontSize:16,flexDirection:'row',alignItems:'center',gap:4},legendDot:{width:8,height:8,borderRadius:4},
 back:{height:42,alignSelf:'flex-start',flexDirection:'row',alignItems:'center',gap:7},backText:{color:colors.muted,fontSize:16,fontWeight:'700'},lead:{color:colors.muted,fontSize:17,lineHeight:28,marginTop:-12},hero:{height:235,maxHeight:235,minHeight:235,flexDirection:'row',alignItems:'stretch',gap:12,overflow:'hidden'},heroCopy:{flex:1,justifyContent:'center',gap:10,minWidth:0},label:{fontSize:16,fontWeight:'900',letterSpacing:1.3},heroTitle:{color:colors.cream,fontSize:21,fontWeight:'800',lineHeight:27},small:{color:colors.muted,fontSize:17,lineHeight:28},bodyFrame:{alignSelf:'center',width:112,height:203,borderRadius:22,borderWidth:1,overflow:'hidden',backgroundColor:colors.deep,position:'relative'},bodyImage:{width:'100%',height:'100%',opacity:.82},previewAxis:{backgroundColor:'rgba(168,217,115,.52)',borderRadius:1,borderStyle:'dashed',borderWidth:.5,height:'82%',left:'50%',position:'absolute',top:'8%',width:1},previewJoint:{borderColor:'#102119',borderRadius:6,borderWidth:1,height:8,marginLeft:-4,marginTop:-4,position:'absolute',width:8},previewJointGood:{backgroundColor:'#A8D973'},previewJointWarn:{backgroundColor:'#D7A85B'},stages:{flexDirection:'row',gap:7},stage:{flex:1,minHeight:132,borderRadius:radii.md,backgroundColor:colors.surface,padding:10,borderWidth:1,borderColor:colors.line},stageIcon:{width:35,height:35,borderRadius:18,borderWidth:1,alignItems:'center',justifyContent:'center'},stageNo:{color:colors.gold,fontSize:16,fontWeight:'900',marginTop:10},stageTitle:{color:colors.cream,fontSize:16,fontWeight:'800',marginTop:2},stageCopy:{color:colors.muted,fontSize:16,lineHeight:13,marginTop:3},report:{flexDirection:'row',alignItems:'center',gap:12},scoreRing:{width:78,height:78,borderRadius:39,borderWidth:3,alignItems:'center',justifyContent:'center'},score:{fontSize:25,fontWeight:'900'},scoreLabel:{color:colors.muted,fontSize:17,fontWeight:'900',letterSpacing:1},reportTitle:{color:colors.cream,fontSize:16,fontWeight:'800'},signal:{fontSize:16,fontWeight:'800'},history:{flexDirection:'row',alignItems:'center',gap:13},historyScore:{fontSize:25,fontWeight:'900'},openHint:{fontSize:16,fontWeight:'800',marginTop:2},detailHeading:{flexDirection:'row',alignItems:'flex-start',justifyContent:'space-between',gap:12},detailSubtitle:{color:colors.muted,fontSize:16,lineHeight:25,marginTop:-7},liveBadge:{borderWidth:1,borderRadius:20,paddingHorizontal:10,paddingVertical:8,flexDirection:'row',alignItems:'center',gap:6},liveBadgeDot:{width:7,height:7,borderRadius:4},liveBadgeText:{fontSize:16,fontWeight:'900',letterSpacing:1},detailTabs:{flexDirection:'row',gap:8},detailTab:{flex:1,borderRadius:14,borderWidth:1,borderColor:colors.line,backgroundColor:colors.surface,paddingVertical:10,alignItems:'center',gap:2},detailTabText:{color:colors.muted,fontSize:16,fontWeight:'800'},detailTabMeta:{color:colors.muted,fontSize:16},detailVisualCard:{padding:12,gap:12},detailVisual:{height:310,borderRadius:18,overflow:'hidden',backgroundColor:'#08110D',position:'relative',alignItems:'center'},detailBodyImage:{height:'100%',width:'70%',opacity:.9},detailAxis:{position:'absolute',top:'8%',bottom:'7%',width:1,opacity:.7},detailJoint:{position:'absolute',width:12,height:12,borderRadius:7,borderWidth:1.5,borderColor:'#07110D',marginLeft:-6,marginTop:-6},detailVisualCopy:{gap:6},detailViewEyebrow:{fontSize:16,fontWeight:'900',letterSpacing:1.4},detailVisualTitle:{color:colors.cream,fontSize:17,fontWeight:'800',lineHeight:26},overallCard:{flexDirection:'row',alignItems:'center',gap:18},overallScore:{width:100,alignItems:'center',justifyContent:'center'},overallValue:{fontSize:46,fontWeight:'900',lineHeight:48},overallSlash:{color:colors.muted,fontSize:16,marginTop:-4},overallState:{fontSize:16,fontWeight:'900',marginTop:5},overallCopy:{flex:1,gap:7},detailSectionTitle:{color:colors.muted,fontSize:16,fontWeight:'900',letterSpacing:1.5},metricRow:{flexDirection:'row',gap:11,alignItems:'center',marginBottom:15},metricIcon:{width:38,height:38,borderRadius:12,borderWidth:1,alignItems:'center',justifyContent:'center'},metricTop:{flexDirection:'row',justifyContent:'space-between',alignItems:'center'},metricName:{color:colors.cream,fontSize:16,fontWeight:'700'},metricValue:{fontSize:17,fontWeight:'900'},metricDetail:{color:colors.muted,fontSize:16},noteCard:{flexDirection:'row',gap:11,alignItems:'flex-start'},noteText:{color:colors.cream,fontSize:16,lineHeight:25},
});
